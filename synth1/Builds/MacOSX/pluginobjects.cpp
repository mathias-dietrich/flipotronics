...
bool CWaveData::readWaveFile(char* pFilePath)
{
    ...
    if(!bFailed && WaveSample.WaveFormatEx.wBitsPerSample == 16)
    {
        ...
        m_pWaveBuffer = new float[nSampleCount];
        int* pShorts = new int[nSampleCount];
        
        memset(pShorts, 0, nSampleCount*sizeof(int));
        int m = 0;
        for(UINT i=0; i<nSampleCount; i++)
        {
            // MSB
            ...
            // LSB
            int lsb = (unsigned char)WaveSample.pSampleData[m];
            // in case top of lsb is bad
            lsb &= 0x00FF;
            pShorts[i] |= lsb;
            m+=2;
    ...
    else if(!bFailed && WaveSample.WaveFormatEx.wBitsPerSample == 32)
    {
        ...
        {
            int* pUSignedInts = new int[nSampleCount];
            memset(pUSignedInts, 0, nSampleCount*sizeof(int));
    ...
    else if(!bFailed && WaveSample.WaveFormatEx.wBitsPerSample == 64)
    {
        ...
        if(WaveSample.WaveFormatEx.wFormatTag == 3) // float
        {
            int* pUSignedLongLongs = new int[nSampleCount];
            memset(pUSignedLongLongs, 0, nSampleCount*sizeof(int));
            ...
            for(UINT i=0; i<nSampleCount; i++)
            {
                ...
                // NSB
                int nsb = (int)WaveSample.pSampleData[m+6];
                // in case top of nsb is bad
                nsb &= mask;
                nsb <<= 48;
                pUSignedLongLongs[i] |= nsb;
                ...
                // NSB2
                int nsb2 = (int)WaveSample.pSampleData[m+5];
                // in case top of nsb is bad
                nsb2 &= mask;
                nsb2 <<= 40;
                pUSignedLongLongs[i] |= nsb2;
                ...
                // NSB3
                int nsb3 = (int)WaveSample.pSampleData[m+4];
                // in case top of nsb is bad
                nsb3 &= mask;
                nsb3 <<= 32;
                pUSignedLongLongs[i] |= nsb3;
                ...
                // NSB4
                int nsb4 = (int)WaveSample.pSampleData[m+3];
                // in case top of nsb is bad
                nsb4 &= mask;
                nsb4 <<= 24;
                pUSignedLongLongs[i] |= nsb4;
                ...
                // NSB5
                int nsb5 = (int)WaveSample.pSampleData[m+2];
                // in case top of nsb is bad
                nsb5 &= mask;
                nsb5 <<= 16;
                pUSignedLongLongs[i] |= nsb5;
                ...
                // NSB6
                int nsb6 = (int)WaveSample.pSampleData[m+1];
                // in case top of nsb is bad
                nsb6 &= mask;
                nsb6 <<= 8;
                pUSignedLongLongs[i] |= nsb6;
